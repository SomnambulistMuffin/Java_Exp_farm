   ...
   readInterestRate();
    ...
    //pnl.add(tArea);
     ...
     rbFemale.setBounds(210, 50, 100, 20);
      txtSal.setBounds(100, 80, 120, 20);
...
//       }
//       monthSal=salary/12;
//       increaseSal=salary+(salary*percIncr);
//       
//       tArea.append("Emp Name: "+txtNm.getText()+"\n Gender: " +gender +
//               "\n Annual Salary: "+ curr.format(salary)+
//               "\n Monthly Salary: "+curr.format(monthSal) +
//               "\n Increased Salary"+ curr.format(increaseSal) +
//               "\n Increment Percentage: "+perc.format(percIncr));
...
objEmp=new EmployeeClass(name, gender, salary,percIncr);
return objEmp;
}

   void readInterestRate(){
   try{
       Scanner rd=new Scanner(new FileReader("interestfile.txt"));
       String rline,strinterest;
       String[]arsplit;
       while(rd.hasNext()){
       rline=rd.nextLine();
       arsplit=rline.split(",");
       strinterest=arsplit[0];
       cmb.addItem(strinterest);
